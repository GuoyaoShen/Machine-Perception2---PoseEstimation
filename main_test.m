...
run('D:\softwares\MATLAB2018a\vlfeat-0.9.21\toolbox\vl_setup.m');
...
U1 = f{1}(1:2,matches(1,:))';  % Nx2
U2 = f{2}(1:2,matches(2,:))';  % Nx2

% Compute calibrated coordinates
% Your code goes here %%%%%%%%%%%%%%%%%%%%%
U1_new = [U1,ones(size(U1,1), 1)]';  % 3xN
U2_new = [U2,ones(size(U2,1), 1)]';  % 3xN
% X1_tem = (inv(K) * U1_new)';   % as a function of U1 and K  Nx3
X1_tem = (K \ U1_new)';   % as a function of U1 and K  Nx3
X2_tem = (K \ U2_new)';   % as a function of U2 and K  Nx3
X1_tem2 = X1_tem ./ X1_tem(:, 3);
X2_tem2 = X2_tem ./ X2_tem(:, 3);
X1 = X1_tem2(:, 1:2);
X2 = X2_tem2(:, 1:2);
